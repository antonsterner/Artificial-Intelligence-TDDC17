...
public class CustomGraphSearch implements SearchObject {
	...
	public ArrayList<SearchNode> search(Problem p) {
		...
		while(!frontier.isEmpty()) {
			...
			for(GridPos childState : p.getReachableStatesFrom(node.getState())) { // getState() returns the current grid position
				SearchNode childNode = new SearchNode(childState, node);
				
				if (!frontier.contains(childNode) && !explored.contains(childNode)) {
					if (insertFront) {
						frontier.addNodeToFront(childNode); // DFS (LIFO)
					} else {
						frontier.addNodeToBack(childNode); // BFS (FIFO)
					}
				}
			}			
		}
