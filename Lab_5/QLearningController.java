	...
	public void tick(int currentTime) {
		...
		if (!paused) {
			...
			if (previous_state != null) {
				...
				reward += StateAndReward.getRewardHover(angle.getValue(), vx.getValue(), vy.getValue());
				double maxAction = getMaxActionQValue(new_state);
				double prevQState = Qtable.get(prev_stateaction); 
				int prevNState = Ntable.get(prev_stateaction);
				
				Qtable.put(prev_stateaction, (prevQState
						+ alpha(prevNState) 
						* (previous_reward + GAMMA_DISCOUNT_FACTOR * maxAction - prevQState)));
